...
def _ensure_waste_table(conn: sqlite3.Connection) -> None:
    """
    建表一次即可：用于记录丢弃（清除）食材的日志，便于后续做浪费统计。
    不影响既有 schema；若不需要统计，可忽略这张表的使用。
    """
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS waste_logs(
            ...
            unit        TEXT,
            reason      TEXT,                -- 例如: 'discard'
            details     TEXT,                -- JSON，记录 consume 返回的批次扣减细节
            created_at  TEXT NOT NULL
        )
        """
    )
...
def discard(
    ...
    """
    清除（丢弃）指定 name 的食材若干数量。
    语义上与 'consume' 等价（都是扣减库存），但我们会额外记一笔 waste_logs。
    返回值与 consume(conn, name, quantity, tz) 一致，并在此基础上完成日志入库。
    """
    # 1) 记录 unit（可能在完全扣空后再查不到，故先取）
    try:
        # 若同模块已有 get_items_by_name，可直接使用；没有也不影响主流程
        items = []
        try:
            items = get_items_by_name(conn, name)  # type: ignore[name-defined]
        except Exception:
            items = []
    ...
    # 2) 调用既有批次扣减逻辑
    result = consume(conn, name, float(quantity), tz)  # type: ignore[name-defined]

    # 3) 写日志（可选，不影响主扣减的成功）
    try:
        _ensure_waste_table(conn)
        now = datetime.now(ZoneInfo(tz)).isoformat()
        conn.execute(
            "INSERT INTO waste_logs(name, quantity, unit, reason, details, created_at) VALUES(?,?,?,?,?,?)",
            (name, float(quantity), unit, reason, json.dumps(result, ensure_ascii=False), now),
        )
        conn.commit()
    except Exception:
        # 日志失败不阻断主流程
